...
% ═══════════════════════════════════════════════════════════════════════════
\subsection{High-Frequency Identification}
...
% ═══════════════════════════════════════════════════════════════════════════
\subsection{Long-Term Projection (LTP)}
...
\]
$g_t$: real GDP YoY; $\text{IP}_t$: Industrial Value Added YoY; $\pi_t$: CPI YoY; $i_t$: FR007; $m_t$: M2 YoY growth;\\ $e_t$: RMB Real Effective Exchange Rate YoY; $y_t^{US}$: US industrial production YoY; \\Variables reflecting the labor market condition were dropped due to quality and availability issues;\\
...
% ═══════════════════════════════════════════════════════════════════════════
\subsection{Counterfactual Solver}
...
\textbf{Terms.} The four terms penalize CPI-target miss, GDP-target miss, non-smooth or abrupt FR007 moves, and exchange-rate instability; the weights $\lambda_\pi,\lambda_y,\lambda_i,\lambda_e$ define the targeting rule; while the discount factor is set to 1 for simplicity following Caravello et al.2025.
...
\qquad
b_{stack} =
\begin{bmatrix}\sqrt{\lambda_\pi} (\pi^* - \pi^{\text{base}}) \\
\sqrt{\lambda_y} (y^* - y^{\text{base}}) \\
\sqrt{\lambda_i} (i_t - i_{t-1}) \\
\sqrt{\lambda_e} (0 - e^{\text{base}})
\end{bmatrix}. 
\]
Algebraically, $\lambda \cdot I = W$ which is the weighting matrix used in the calculation of $\hat{A} = M \cdot W$ for the rules written in the form of $\hat{A}_{x} \tilde{x} +\hat{A}_{z} \tilde{z} = 0$ used in Caravello et al. 2025.
...
\end{center}
\scriptsize The first row corresponds to the strict CPI targeting with a smoothness penalty on interest rate changes: $\lambda_{\pi}=\lambda_i=1$;\\
\scriptsize The second row adds the penalty for GDP deviation: $\lambda_{\pi}=\lambda_y=\lambda_i=1$;\\
\scriptsize The third row adds the penalty for exchange rate volatility: $\lambda_{\pi}=\lambda_y=\lambda_i=\lambda_e=1$.

\textcolor{spgray}{\scriptsize\hyperlink{app:cnfctl2025}{\beamergotobutton{Counterfactual paths using data until 2025}}}

\end{frame}
 
\begin{frame}{Takeaways}
\begin{itemize}
      \item Across all three targeting rules, the price puzzle is uniformly present. In the first one with only CPI targeting, a persistent rate cuts compared to the forecast is required to hit the CPI target, but as a result the GDP would be higher than the baseline and the target. With both CPI and GDP targeting, a sustatining rate increased is necessary. As a result, however, the industrial output would suffer a sharp decline. With the addition of exchange rate stability rule, the rate path is similar to the forecast, alongside output and inflation. Still, the interest rate and exchange rate  in all three rules is higher than the reality, which is not something that the government would be happy to see.
      \begin{itemize}
            \item Unfortunately, as mentioned before, since net export, consumption and investment are used in the monthly GDP construction, we did not have it in the reduced form forecast and thus cannot calculate the counterfactual path to analyze the effects on them.
      \end{itemize}
      \item The reduced-form baseline forecast from 2023 onward using data until 2022 actually did a good job in the predicting the GDP growth and the industrial output, while the CPI forecast is much higher than the reality, and this issue is only partially resolved by using the data until 2025, suggesting that the model is not able to capture the structural changes in the economy. 
\end{itemize}
\end{frame}
...
\textbf{1. Fragmented fiscal authority across regions}
\begin{itemize}\itemsep0.1em
  \item Caravello et al.\ (2025) show that for the US, the empirics-only
        counterfactual is close to those produced by models like RANK and HANK.
  \item This relies on a unified fiscal--monetary stance: the IRFs span the
        relevant policy frontier because there is one fiscal authority behind
        them. In China (provinces, LGFVs) and the euro area (member states,
        NGEU), the IRFs we estimate average over heterogeneous fiscal
        actors, and the spanning argument is no longer clean.
\end{itemize}

\textbf{2. Fiscal Regime Changes}
\begin{itemize}\itemsep0.1em
  \item Caravello et al.\ assumed fiscal policy fixed along the counterfactual
        monetary path. In China this is hard to defend: the 2014 budget law
        reform, LGFV debt swaps, the 2019 VAT cuts, and the 2024 local-government
        debt resolution all shifted the fiscal stance materially within the sample.
\end{itemize}
\end{frame}
\begin{frame}{Limitations and Extensions (cont'd)}
\vspace{0.2em}
\textbf{3. Where I would like to go next.}
\begin{itemize}\itemsep0.1em
  \item Both extensions point back to the question that motivated this thesis
        in the first place: what actually determines the price level
        when the monetary--fiscal split is non-standard?
      \vspace{0.3em}\\

\begin{itemize}
  \item[$\bullet$] \textbf{Monetary--fiscal interaction.}  The central bank takes the fiscal stance as given, the fiscal
        authority takes the rate path as given, and the price level is
        determined jointly.
  \item[$\bullet$] \textbf{Fiscal--fiscal interaction.}  When ``fiscal'' is itself a collection of
        actors, central vs.\ provincial in China, federal-like vs.\
        member-states in the euro area, their strategic interaction shapes
        the aggregate fiscal stance that the central bank then takes as given.
\end{itemize}
  \item The euro area is the natural next laboratory: institutionally between
        the unified US benchmark and the Chinese case, and with the data
        quality needed to credibly identify the joint monetary--fiscal object.
\end{itemize}
...
to growth is hypothesised to be asymmetric. MANR2026 further estimated this threshold at around 1pp, meanining that the PBoC tends to tolerate small overshoots in output. 
...
\textbf{Stock \& Watson (2010) distribution method}, adapted to NBS quarterly GDP using the sum identity $Q_T = q_{3T-2} + q_{3T-1} + q_{3T}$, with monthly indicators Consumption / Fixed Assets Investment/ Government Spending / Trade Balance.

\begin{center}
\includegraphics[width=0.9\textwidth,height=0.6\textheight,keepaspectratio]{outputs/diagnostics/gdp_quarterly_vs_proxy_monthly.png}
\end{center}
...
\textbf{Blue.} IRF of Chinese economy to monetary policy shocks identified from FR007 surprises on \emph{any} PBoC policy announcement day.
\textbf{Red.} IRF using \emph{rate-change-only} dates.


\begin{center}
\includegraphics[width=0.92\textwidth,height=0.6\textheight,keepaspectratio]{outputs/main_results/2022/irf_hfi_comparison.png}
\end{center}
...
\textbf{Specification choice.} Comparison of policy indicators' deviations using FR007 versus CMPI (MANR2026).

\begin{center}
\includegraphics[width=0.9\textwidth,height=0.6\textheight,keepaspectratio]{outputs/robustness/cmpi_vs_fr007.png}
\end{center}


\textcolor{spgray}{\scriptsize The choice to retain FR007 can be also attributed to the fact that it contains enough variations, though less than CMPI, to identify the narrative shocks, and it is more directly linked to the HFI shocks identified from FR007 surprises.}

\hyperlink{slide:rr2}{\beamerreturnbutton{back}}
\end{frame}

\begin{frame}{Appendix ---Headline Results with Data Until 2025}
\hypertarget{app:cnfctl2025}{}
...
\end{center}
\hyperlink{slide:cnfctl2}{\beamerreturnbutton{back}}
