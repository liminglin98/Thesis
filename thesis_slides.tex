...
\begin{itemize}
      \item Across all three targeting rules, FR007 and CPI move in the same direction, more aggressive rate cuts achieve lower inflation, consistent with the price puzzle identified in the IRFs. The tradeoff is that stricter CPI targeting comes at the cost of larger NEER swings and greater output volatility relative to more balanced rules.
      \begin{itemize}
      ...
      \end{itemize}
      \item The reduced-form baseline forecast is notably pessimistic from 2023 onward, likely reflecting the BVAR extrapolating post-COVID deflationary dynamics that may represent a structural break from the estimation sample. While counterfactual paths generally improve on the baseline for both CPI and output, the more relevant comparison is between the counterfactual and the GWR targets, and the persistent gap between the two is the central finding.
\end{itemize}
\end{frame}
