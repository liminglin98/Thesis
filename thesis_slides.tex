...
\subtitle{A Counterfactual Analysis Using the Wolf et al.\ (2025) Framework}
\author{Liming Lin}
...
the PBoC had strictly followed an inflation-targeting rule during this
period?
...
\node[box, right=of hfi] (ltp) {\textbf{(3) Long Term Projections}\\ \scriptsize Reduced Form VAR estimated with 7-variable and Minnesota-prior};
...
\begin{itemize}\itemsep0.2em
  ...
  \item Block (4) solves for the shock sequence that moves the baseline
        toward an inflation-targeting path, subject to a multi-objective
        loss function.
\end{itemize}
...
\begin{itemize}\itemsep0.15em
  ...
  \item[$\bullet$] $\text{gdp\_gap}_t = g_t - g^{*}_t$, where $g_t$ is monthly real GDP YoY
  ...
  \item[$\bullet$] $\text{fx\_gap}_t$: post-2006 interaction between the change
        in the central parity rate and the spot--parity deviation
\end{itemize}
\end{frame}
...
\begin{itemize}\itemsep0.1em
  \item[$\circ$] CRZ/Chen kept FR007 as the dependent variable but excluded the
        exchange rate; MANR2026 added the FX interaction but used the CMPI
        composite on the left-hand side.
  \item[$\circ$] We retain FR007 (the actual market rate that anchors HFI surprises
        downstream) while adopting MANR2026's FX specification.
\end{itemize}
...
\[
\text{shock}_t \;=\; \bigl(\text{FR007}_t - \text{FR007}_{t-1}\bigr) \cdot \mathbb{1}\{t \in \mathcal{E}\}
\]

where $\mathcal{E}$ is the set of policy event dates.

\vspace{0.4em}
\textbf{Core policy instruments tracked.} Event dates are hand-collected from PBoC announcements (compiled in CMPI.xlsx, Miranda-Agrippino, Nenova \& Rey 2026):
...
\]
$g_t$: real GDP YoY (Stock--Watson distribution from quarterly NBS GDP, \hyperlink{app:gdp}{\beamergotobutton{appendix}}); $\text{IP}_t$: industrial production YoY; $\pi_t$: CPI YoY; $i_t$: FR007; $m_t$: M2 YoY growth; $e_t$: RMB NEER YoY; $y_t^{US}$: US industrial production YoY.
}

\end{frame}
\begin{frame}{Reduced-Form Long-Term Projection: Baseline and MA Representation}
\vspace{0.3em}
	extbf{Estimation samples.}
\begin{itemize}\itemsep0.1em
      \item Monthly estimation with COVID dummies included in all runs.
      \item For each projection exercise, parameters are estimated through $t^*-1$.
      \item Baseline vintages reported for 2019, 2022, and 2025.
\end{itemize}
...
\qquad h=1,2,\ldots,H
\]
	extbf{No data from the projection window enters the forecast.} For $\ell<h$, $\hat{y}_{t^*+h-\ell}$ is previously forecast; for $\ell\ge h$, it is pre-$t^*$ realized data.

\vfill
	extbf{Stored for counterfactual stage:} Wold MA form $y_t = c^* + \sum_{h=0}^{120} \Psi_h\, u_{t-h}$.
...
\vspace{0.5em}
% Placeholder images for FR007 fit and residuals
% Top: Actual vs predicted FR007
% Bottom: Residual series epsilon_t^MP
...
\vspace{0.5em}
% Placeholder image for HFI IRF comparison
% Left: any announcement shock IRF
% Right: rate-change-only shock IRF
