...
\sqrt{\lambda_i} (\tilde i_{t-1} - i^{\text{base}}) \\
\sqrt{\lambda_e} (0 - e^{\text{base}})
\end{bmatrix}. 
\]
Algebraically, $\lambda \cdot I = W$ which is the weighting matrix used in the calculation of $\hat{A} = M \cdot W$ for the rules written in the form of $\hat{A}_{x} \tilde{x} +\hat{A}_{z} \tilde{z} = 0$ used in Caravello et al. 2025 and McKay \& Wolf (2023).
