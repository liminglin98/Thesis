...
_Third_, the paper documents and rationalizes a robust price puzzle in Chinese CPI responses to monetary policy shocks. The interpretation through the housing user-cost channel, building on #cite(<MirandaAgrippinoNenovaRey2026>, form: "prose"), has implications for the design of future inflation-targeting frameworks in China and for the cross-country literature on monetary transmission to housing.
...
The structural approach overcomes the Lucas-critique vulnerability, but at a substantial cost: counterfactual conclusions depend on which structural model is chosen, and the relevant choices, including the form of household heterogeneity, the degree of market incompleteness, the nature of nominal and real frictions, are difficult to discipline from the aggregate data alone. Different specifications can deliver materially different counterfactual answers to the same policy question.
...
The standard approach to evaluating monetary policy counterfactuals since #cite(<Lucas1976>, form: "prose") has been fully structural: specify a model, change the policy rule, and re-solve. The sufficient-statistics approach of #cite(<McKayWolf2023>, form: "prose"), and #cite(<CaravelloEtAl2026>, form: "prose") (henceforth CMW) offers an alternative that sidesteps the need for a complete structural model while remaining robust to the Lucas critique.

The key distinction from earlier VAR-based counterfactual exercises, such as #cite(<SimsZha1995>, form: "prose") and #cite(<BGW1997>, form: "prose"), is in how the counterfactual policy is implemented. The earlier literature adjusted the VAR coefficients to reflect an alternative policy rule, which is vulnerable to the Lucas critique: agents' behavior and expectations would change under the new rule, invalidating the original reduced-form coefficients. CMW instead hold the estimated impulse response functions fixed and treat the policy rule change as a linear combination of monetary policy shocks at each period. The counterfactual path is then simply the baseline forecast plus the transmission map applied to a shock sequence that minimizes a loss function encoding the alternative rule. Because the shocks are drawn from the space of historically observed policy surprises, the approach does not require agents to form expectations about a new regime, the counterfactual is constructed as if the central bank had made a different sequence of choices within the prevailing framework.

This is the sense in which the method overcomes the Lucas critique: the IRFs from the prevailing rule remain valid as long as the counterfactual stays within the span of historically observed policy movements, that is, as long as the counterfactual represents a more aggressive or more restrained use of existing instruments, rather than a fundamental regime change. We discuss the validity of this linearity assumption in more detail below.
...
The CMW framework rests on several assumptions, drawn from #cite(<McKayWolf2023>, form: "prose") and CMW (2026). Several of these are non-trivial in the Chinese institutional setting. We discuss each in turn, treating violations as scope conditions for our results rather than reasons to reject the approach outright.
...
*Single policy instrument.* CMW assumes that the central bank operates through a single instrument whose transmission can be captured by a single set of IRFs. This is the most questionable assumption for the Chinese case, given the PBoC's multi-instrument toolkit. Our approach is to treat FR007 as a sufficient summary statistic for the overall policy stance: although the PBoC operates through multiple tools, OMOs, RRR adjustments, MLF, LPR, and formerly benchmark rates, all of these ultimately transmit through the interbank market and are reflected in FR007 movements. The counterfactual is therefore interpreted as: what would have happened if the PBoC had generated a different path for the short-term interbank rate, regardless of which specific instrument was used to achieve it. We provide a robustness check using the CMPI of #cite(<MirandaAgrippinoNenovaRey2026>, form: "prose") (henceforth MANR) as an alternative policy indicator.
...
Event dates are sourced from PBoC announcements collected via Wind. We restrict to dates on which a rate _change_ occurs — dates with announcements confirming no change are excluded, as they generate minimal market surprise in the close-to-close window.
...
The estimated IRFs exhibit a positive CPI response to contractionary monetary policy shocks, a price puzzle. We obtain this pattern under both narrative and HFI identification, consistent with the findings of MANR (2026). We return to its interpretation and counterfactual implications below.
...
Quarterly real GDP is the only available frequency from the NBS, but quarterly data produces unreliable narrative shock identification. MANR (2026) use monthly data estimated by the New York Fed (Groen, 2020), which employs a dynamic factor model on dozens of indicator series. However, that series ends around 2020 and is therefore not suitable for our 2023-2025 counterfactual window.

As a result, we adopt the #cite(<StockWatson2010>, form: "prose") distribution method, which allocates quarterly GDP to months using the adding-up identity $Q_T = q_(3T-2) + q_(3T-1) + q_(3T)$ and monthly indicator series that track the expenditure components. The method fits a state-space model with a Kalman smoother, ensuring that the monthly estimates sum exactly to the quarterly total reported by the NBS.

Monthly indicators used for distribution:

- _Consumption:_ Monthly total retail sales of consumer goods (社会消费品零售总额), in absolute value. Source: NBS.
- _Investment:_ Urban fixed asset investment (城镇固定资产投资额). The overall FAI series is available only in growth rate form; we use the earliest available urban FAI in levels and backward-extend using the overall FAI growth rate. Note that local government capital spending, which passes primarily through FAI channels, is captured here, partially alleviating the problem that the government spending series below covers only central government expenditure.
- _Government spending:_ Monthly central government fiscal expenditure (国家财政支出). This series excludes local government expenditure and transfer payments. The omission of local spending is mitigated by its inclusion in FAI (above), but the omission of transfers remains a limitation.
- _Net exports:_ Monthly merchandise trade balance (exports minus imports), recorded in USD. Converted to CNY using the monthly average CNY/USD spot rate constructed from daily data. 

- _CPI:_ Monthly CPI index, used to deflate the constructed nominal monthly GDP into real terms.

For level series reported only as an aggregated January--February observation, the combined value is divided equally across the two months by assigning the January--February average to both January and February before the Stock--Watson distribution.
...
- _BIS Real Effective Exchange Rate:_ Constructed as a trade-weighted exchange-rate index against major partner currencies, adjusted for relative consumer-price movements across China and its trading partners.
...
- _January--February growth-rate observations:_ When a growth rate is reported only for January--February jointly, the same reported value is assigned to both January and February.
...
- _2020 (no target):_ The government issued no GDP growth target for 2020 due to COVID. In months without a newly announced target, we carry forward the previously operative target value, so the target series remains defined continuously through the no-target period. The target-series construction is documented in Appendix @app:gwr-targets.
...
The most restrictive specification ($lambda_y = 0, lambda_e = 0$) targets only CPI deviation from the GWR target, subject to rate smoothing. Here, FR007 needs to be lower than the BVAR forecast (which projects rates above 2.25%) but remains higher than the realized path. GDP overshoots to approximately 7%. The NEER appreciates above the baseline. IP remains close to both the forecast and the actual path,as the FR007 deviation from the baseline forecast is relatively small under this specification, the transmission into IP is muted.
...
A post-2023 caveat applies to all three specifications. The BVAR baseline inherits the sample's central tendency, which is that inflation averaging well above zero, and therefore forecasts CPI around 1.75% even as realized CPI collapsed. The counterfactual CPI paths are thus computed relative to an already-elevated baseline, which means the level of counterfactual CPI should be read with some caution. Even under the full specification, the counterfactual CPI remains well below the GWR target. The counterfactual FR007 path stays near the BVAR forecast, which is substantially above the realized rate path, implying that the optimizer finds no benefit in the aggressive easing the PBoC actually pursued, because the exchange rate and output constraints offset any CPI gains from further cuts. The 2023-2025 inflation undershoot thus reflects structural limits on rate-based transmission within the PBoC's multi-objective mandate: the central bank cut more than the counterfactual prescribes, yet could not close the gap to its stated CPI target.
...
The main specification treats FR007 as a sufficient statistic for the PBoC's policy stance, on the argument that all instruments — OMOs, RRR, MLF, LPR, and formerly benchmark rates — ultimately transmit through the interbank market and are reflected in FR007 movements (§4.2.3). This argument is most convincing in the post-2015 era, when the 7-day reverse repo rate became the PBoC's explicit policy anchor and FR007 responded directly to OMO rate changes. In the pre-2015 period, when the PBoC operated primarily through quantity-based tools (RRR adjustments, credit quotas) and administratively set benchmark rates, the extent to which these actions transmitted into FR007 is less clear — FR007 may have captured liquidity conditions without fully reflecting the real-economy transmission of quantity-based easing or tightening.
