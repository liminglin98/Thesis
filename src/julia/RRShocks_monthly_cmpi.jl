...
function print_policy_rule_summary(model, label)
    ct = coeftable(model)
    println("\nPolicy-rule coefficients ($(label))")
    println(@sprintf("  %-14s %12s %12s %10s %12s %12s %12s",
        "term", "coef", "se", "t", "p_value", "ci_low", "ci_high"))
    println("  " * "-"^88)
    for i in eachindex(ct.rownms)
        println(@sprintf("  %-14s %+12.6f %12.6f %+10.3f %12.4g %+12.6f %+12.6f",
            ct.rownms[i],
            ct.cols[1][i],
            ct.cols[2][i],
            ct.cols[3][i],
...
print_policy_rule_summary(model1, s.label)
...
var_syms = [:realgdp_monthly_yoy, :cpi, :cmpi, :FR007, :neer_yoy, :IP_yoy]
...
var_labels = ["Real GDP Growth", "CPI", "CMPI", "FR007", "Real Effective Exchange Rate", "Industrial Value Added"]
...
display(plot_bvar)
savefig(plot_bvar, joinpath(irf_out_dir, cmpi_artifact_name("irf_bvar_iv_svar.png")))
...
# Save
serialize(joinpath(INTER_DIR, NARRATIVE_RESULTS_FILE), Dict(
    "irf_point" => irf, "irf_draws" => irf_draws, "H" => H))

println("Results saved to $(joinpath(INTER_DIR, NARRATIVE_RESULTS_FILE))")
