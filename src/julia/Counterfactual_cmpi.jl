...
#
# Method: minimize weighted loss L with neutral CMPI regularization, solved in closed form.
#   L = λ_π ‖π - π*‖² + λ_y ‖y - y*‖² + λ_i ‖i‖² + λ_e ‖E_m ν‖²
#
# Three targeting rules (all include CMPI close-to-zero regularization):
...
const LTP_DATA_FILE = "china_longterm_data_cmpi.csv"
const BVAR_RESULTS_FILE = "bvar_results_cmpi.jls"
const NARRATIVE_RESULTS_FILE = "narrative_irf_results_cmpi.jls"
const COUNTERFACTUAL_RESULTS_FILE = "counterfactual_results_cmpi.jls"
...
#   1: Real GDP YoY   2: IP YoY   3: CPI YoY   4: CMPI
...
# IRF ordering (RRShocks_monthly.jl): 6 variables
#   1: GDP   2: CPI   3: CMPI   4: FR007   5: NEER   6: IP
const irf_gdp_idx   = 1
const irf_cpi_idx   = 2
const irf_cmpi_idx  = 3
const irf_fr007_idx = 4
const irf_neer_idx  = 5
const irf_ip_idx    = 6
...
_df = CSV.read(joinpath(DERIVED_DIR, LTP_DATA_FILE), DataFrame)
_df.date = Date.(_df.date); sort!(_df, :date)
_syms = [:realgdp_monthly_yoy, :IP_yoy, :cpi, :cmpi, :M2_growth, :neer_yoy, :US_IP_yoy]
...
function cnfctl_flexible(pi_base, y_base, i_base,
    ...
    # CMPI is a standardized trend-deviation index, so the regularization target
    # is a neutral policy stance at zero, not a smoothed path around last month.
    gap_i = -i_base
    A_i = I_m
...
function cnfctl_flexible_evol(Y, A_list, c,
    ...
    for t_simul in 1:max_roll
        ...
        i_base_t  = baseline_t[:, cmpi_idx]
        e_base_t  = baseline_t[:, neer_idx]
        ip_base_t = baseline_t[:, ip_idx]

        Pi_t = build_transmission_map(narr_point[:, irf_cpi_idx],   H_t)
        Y_t  = build_transmission_map(narr_point[:, irf_gdp_idx],   H_t)
        I_t  = build_transmission_map(narr_point[:, irf_cmpi_idx], H_t)
        E_t  = build_transmission_map(narr_point[:, irf_neer_idx],  H_t)
        IP_t = build_transmission_map(narr_point[:, irf_ip_idx],    H_t)
...
function save_rule_figure(method::String, sample::String, rule_label::String,
    ...
    col_vars = [
        ...
        (idx=cmpi_idx, base=i_base,  cf=i_cf,  tgt=nothing,    ctitle="CMPI"),
    ...
    fpath = joinpath(out_dir, "cnfctl_cmpi_$(method)_$(rule_slug(rule_label))_s$(sample).png")
    savefig(fig, fpath)
    return fpath
end
...
function posterior_bands_flex(pi_base, y_base, i_base, ip_base,
                              narr_draws, n_draws_irf, fcst_hor,
                              pi_target, y_target,
                              λ_pi, λ_y, λ_i, λ_e)
    ...
    for d in 1:n_draws_irf
        Pi_d  = build_transmission_map(narr_draws[d,:,irf_cpi_idx],   fcst_hor)
        Y_d   = build_transmission_map(narr_draws[d,:,irf_gdp_idx],   fcst_hor)
        I_d   = build_transmission_map(narr_draws[d,:,irf_cmpi_idx], fcst_hor)
        E_d   = build_transmission_map(narr_draws[d,:,irf_neer_idx],  fcst_hor)
        IP_d  = build_transmission_map(narr_draws[d,:,irf_ip_idx],    fcst_hor)
        try
...
# Targeting rules (rows in each figure): all include CMPI close-to-zero regularization
rules = [
...
for yc in year_configs
    ...
    # --- Load BVAR baseline ---
    bvar = deserialize(joinpath(INTER_DIR, BVAR_RESULTS_FILE))
    A_list     = bvar["A_list"]
    c_vec      = bvar["c"]
    ...
    # --- Load RR/narrative IRFs ---
    narr_data  = deserialize(joinpath(INTER_DIR, NARRATIVE_RESULTS_FILE))
    ...
    n_draws_irf = size(narr_draws, 1)

    println("IRFs: H=$H_irf, $n_draws_irf RR/narrative draws")
    ...
    i_base  = baseline_fc[:, cmpi_idx]
    e_base  = baseline_fc[:, neer_idx]
    ip_base = baseline_fc[:, ip_idx]
    println("  CMPI base[1] = $(i_base[1]), neutral target = 0.0")
    pi_target = [cpi_target_for_date(d) for d in cnfctl_dates]
    y_target  = [gdp_target_for_date(d) for d in cnfctl_dates]

    Pi_m = build_transmission_map(narr_point[:, irf_cpi_idx],   fcst_hor)
    Y_m  = build_transmission_map(narr_point[:, irf_gdp_idx],   fcst_hor)
    I_m  = build_transmission_map(narr_point[:, irf_cmpi_idx], fcst_hor)
    E_m  = build_transmission_map(narr_point[:, irf_neer_idx],  fcst_hor)
    IP_m = build_transmission_map(narr_point[:, irf_ip_idx],    fcst_hor)
    ...
    for rule in rules
        ...
        println(@sprintf("  Loss: total=%.1f  π=%.1f  y=%.1f  cmpi=%.1f  e=%.1f",
        ...
        println(@sprintf("  CMPI:  mean=%.2f  range=[%.2f, %.2f]",
    ...
    # --- Build combined n_rules×5 figure (rows=rules, cols=CPI/GDP/CMPI/NEER/IP) ---
    blue  = RGB(0.45, 0.62, 0.70)
    ...
    col_vars = [
        ...
        (idx=cmpi_idx,  cnfctl=:i_cnfctl,  med=:i_med,  base=i_base,  e_adj=false, lb=:i_lb,  ub=:i_ub,  tgt=nothing,    col_title="CMPI"),
    ...
    display(fig)
    fname_scenario = "cnfctl_cmpi_scenario_$(cfctl_year)_s$(yc.sample).png"
    ...
    if length(cmp_results) == 2
        ...
        display(fig_cmp)
        fname_cmp = "cnfctl_cmpi_scenario_compare_focus_$(cfctl_year)_s$(yc.sample).png"
    ...
    # --- Build combined n_rulesx5 figure for evol (rows=rules, cols=CPI/GDP/CMPI/NEER/IP) ---
    ...
    display(fig_evol)
    fname_evol = "cnfctl_cmpi_evol_$(cfctl_year)_s$(yc.sample).png"
...
serialize(joinpath(intermediate_dir("2025"), COUNTERFACTUAL_RESULTS_FILE), Dict(
    "results" => all_results,
