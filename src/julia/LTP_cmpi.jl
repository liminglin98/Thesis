...
# Variables: gdp_yoy, ip_yoy, cpi_yoy, cmpi, m2_yoy, neer_yoy, usip_yoy
...
function cmpi_artifact_name(base::String)
    ...
    return "$(root)_cmpi$(ext)"
end

# ── Load data once ───────────────────────────────────────────────────────────
df_raw = CSV.read(joinpath(DERIVED_DIR, LTP_DATA_FILE), DataFrame)
...
var_syms  = [:realgdp_monthly_yoy, :IP_yoy, :cpi, :cmpi, :M2_growth, :neer_yoy, :US_IP_yoy]
var_labels = ["Real GDP", "IP", "CPI", "CMPI", "M2", "NEER", "US IP"]
n = length(var_syms)
...
display(p_wold)
savefig(p_wold, joinpath(DIAG_DIR, cmpi_artifact_name("bvar_wold_decay.png")))
##
p_resid_plots = []
for j in 1:n
...
display(p_resid)
savefig(p_resid, joinpath(DIAG_DIR, cmpi_artifact_name("bvar_residuals.png")))
##
gdp_idx = 1
...
display(p_gdp)
savefig(p_gdp, joinpath(DIAG_DIR, cmpi_artifact_name("bvar_gdp_forecast.png")))

cpi_idx = 3
# Match Counterfactual_cmpi.jl: the baseline path used in the counterfactuals
# is a recursive forecast starting from the last observed month before 2023.
...
CSV.write(joinpath(DIAG_DIR, cmpi_artifact_name("cpi_forecast_vs_actual_until_2026.csv")), cpi_plot_df)
...
display(p_cpi)
savefig(p_cpi, joinpath(DIAG_DIR, cmpi_artifact_name("cpi_forecast_vs_actual_until_2026.png")))
##
# ── Save results ─────────────────────────────────────────────────────────
results = Dict(
    "A_list"         => A_list,
...
serialize(joinpath(INTER_DIR, BVAR_RESULTS_FILE), results)
println("\nResults saved to $(joinpath(INTER_DIR, BVAR_RESULTS_FILE))")
