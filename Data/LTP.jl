...
# ── 1) Load & prepare data ───────────────────────────────────────────────────
df = CSV.read("china_longterm_data.csv", DataFrame)
...
# A_list[L] is n×n: coefficient on lag L (rows = equations, cols = variables)
function get_lag_matrices(B::Matrix{Float64}, n::Int, p::Int)
    [Matrix(transpose(B[1 + (L-1)*n+1 : 1 + L*n, :])) for L in 1:p]
end
...
display(p_wold)
savefig(p_wold, "bvar_wold_decay.png")
##
# Residuals
p_resid_plots = []
for j in 1:n
...
display(p_resid)
savefig(p_resid, "bvar_residuals.png")
##
...
display(p_gdp)
savefig(p_gdp, "bvar_gdp_forecast.png")
##
# ── 13) Save results ──────────────────────────────────────────────────────────
results = Dict(
    "A_list"         => A_list,
...
serialize("bvar_results.jls", results)
println("\nResults saved to bvar_results.jls")
